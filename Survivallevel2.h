...
public:
    SurvivalLevel2(World* w, sf::Font& f)
        : map(64, 1600, 900)
    {
        world = w;
        player = NULL;
    ...
    void draw(sf::RenderWindow& window, sf::Text& dmgText)
    {
        ...
        if (levelClear)
        {
            msgText.setString("LEVEL 2 CLEAR!\n\nPress Enter to Continue\nPress Escape for Menu");
            window.draw(msgText);
            return;
        }
        ...
        char buf[100] = "";
        sprintf_s(buf, "LEVEL 2 - DESERT   Kills: %d / %d     X:Shoot  Z:Grenade  Esc:Menu", killCount, killsNeeded);
